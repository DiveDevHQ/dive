...
export function getAppTemplates(app, account_id) {
    return axios
...
export function getAccountTemplates(account_id) {
    return axios
        .get(`${serviceUrl}/templates/${account_id}`)
        .then(res => res.data)
        .catch(err => console.log(err));
}
...
export function queryData(account_id, connector_id,obj_type, query_text, top_k,instruction,query_type) {

    return axios.get(`${serviceUrl}/api/v1/documents/search?query_text=${query_text}&account_id=${account_id}&connector_id=${connector_id}&obj_type=${obj_type}&top_k=${top_k}&instruction=${instruction}&query_type=${query_type}`)
        .then(res => res.data);
}
